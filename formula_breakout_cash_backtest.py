...
def simulate(args: argparse.Namespace) -> Dict[str, object]:
    ...
    report = {
        ...
        "lot_size": lot_size,
        "assumption": "每天按收盘价检查已有仓位，日线回测用当日收盘价近似；T+1交易，买入日当日不卖出；后续交易日优先判断止损，若收盘价低于买入日收盘价则按收盘价止损卖出；未触发止损时，放量阴线、阴十字星、连续两根阴线任一成立即按收盘价止盈卖出。随后按当日公式评分排名以收盘价买入1手；买入时刻按收盘集合竞价/15:00近似；已计入佣金、印花税、过户费；不计滑点和真实排队成交。",
        "fee_model": {
        ...
        "sell_rules": {
            ...
            "stop_loss": "止损优先：后续交易日收盘价低于买入日收盘价时，按当日收盘价止损卖出。",
