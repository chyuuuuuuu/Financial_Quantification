...
def simulate(args: argparse.Namespace) -> Dict[str, object]:
    ...
    for date_text in market_dates:
        ...
        for holding in holdings:
            ...
            pnl = proceeds - holding.entry_price * holding.shares
            cash += proceeds
            realized_pnl += pnl
            total_sells += 1
            ret_pct = (price / holding.entry_price - 1.0) * 100.0 if holding.entry_price > 0 else 0.0
        ...
        for holding in holdings:
            ...
            unrealized_pnl += (close - holding.entry_price) * holding.shares
        ...
        open_holdings.append(
            {
                **asdict(holding),
                ...
                "market_value": round(close * holding.shares, 2),
                "unrealized_pnl": round((close - holding.entry_price) * holding.shares, 2),
                "unrealized_ret_pct": round((close / holding.entry_price - 1.0) * 100.0, 4) if holding.entry_price > 0 else 0.0,
            }
        )
    ...
    report = {
        ...
        "lot_size": lot_size,
        "assumption": "每天收盘先按当日K线检查已有仓位卖出，再按当日公式评分排名以收盘价依次买入1手；不计手续费、滑点和涨跌停无法成交影响。",
        "sell_rules": {
