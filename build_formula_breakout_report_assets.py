...
def build_limitations(report: Dict[str, Any], coverage: Dict[str, Any]) -> List[str]:
    ...
    return [
        ...
        "未触发止损时先判断均线排列：MA5>MA10>MA20为多头严格模式，否则为非多头简化模式。",
        "多头严格模式：大跌阴线、阴包阳、阴十字星、放量阴线、长上影阴线、连续两阴任一出现，都必须同时收盘跌破MA5才卖出。",
        "非多头简化模式：阴十字星、放量阴线或连续两阴任一出现即按收盘价卖出，无需参考MA5。",
        "买入时刻按信号日收盘集合竞价/15:00的收盘价近似。",
        "量能口径：选股三倍阳量为V>3*REF(V,1)且C>O；缩量为MA5/MA10成交量低于三倍阳当日量。",
        "严格模式放量阴线为收阴、实体跌幅>2%、成交量大于前一日1.5倍；简化模式放量阴线为收阴、实体跌幅>2%、成交量大于昨日。",
        "大跌阴线为当日收阴且日跌幅<-6%；连续两根阴线为当日C<O且前一交易日C<O。",
