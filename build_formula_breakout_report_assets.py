...
def build_limitations(report: Dict[str, Any], coverage: Dict[str, Any]) -> List[str]:
    ...
    limitations = [
        "卖出规则按用户最新指定执行：T+1交易，买入日不卖出；后续交易日只在收盘时判断止损和止盈。",
        "止损优先：后续交易日收盘价低于买入日收盘价*0.95时，按当日收盘价止损卖出；若不触发止损，再判断止盈信号。",
        "止盈条件：放量阴线、阴十字星、连续两根阴线任意一个成立即按收盘价止盈卖出；长上影阴线、阴包阳不卖出。",
        "买入时刻按信号日收盘集合竞价/15:00的收盘价近似。",
        "量能口径：选股三倍阳量为V>3*REF(V,1)且C>O；缩量为MA5/MA10成交量低于三倍阳当日量。",
        "卖出里的放量阴线为C<O、实体跌幅(open-close)/open>2%、且V>REF(V,1)。",
        "阴十字星为C<O且实体占全日最高至最低振幅比例小于20%。",
...
def build_trade_payload(report: Dict[str, Any], trades: Dict[int, Dict[str, Any]], minute_counts: Dict[str, int], publish_revision: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "minute_data": {
            ...
            "note": "当前卖出规则以日线收盘信号判断，交易明细不补充盘中首次触发分钟。",
